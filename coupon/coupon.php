...
function uka_enable()
{
    ...
    if(empty($check_status)) {#若表單檢查無誤 將會執行安裝
       $content_type = array(
            'type' => 'coupon',
            'name' => '折價卷',
            'description' => '',
        ...
        //可用選單 留空即可
        variable_set("menu_options_{$content_type['type']}", array());
        //預設回應 2開啟 1關閉
        variable_set("comment_{$content_type['type']}", '1');

        //其餘預設值設定
        variable_set("comment_default_per_page_{$content_type['type']}", '50');
        variable_set("comment_anonymous_{$content_type['type']}", 0);
        variable_set("comment_default_mode_{$content_type['type']}", 1);
        variable_set("comment_form_location_{$content_type['type']}", 1);
        variable_set("comment_subject_field_{$content_type['type']}", 1);
...
##最後將所有field與coupon node建立 instance
        ...
        dpm('field資料有錯誤！將不會進行欄位建立！錯誤名單如下');
        dpm($check_status);
    }
